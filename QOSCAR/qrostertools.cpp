...
void QRoster::handleItems(const QByteArray &baItems)
{
    ...
    mutex.lock();
    bool ok;
    QByteArray baData(baItems), baTemp;
    ...
    baTemp = baData.mid(0, 2);
    quint16 u16CLCount =  QString(baTemp.toHex()).toUInt(&ok, 16); // CL Items in packet
    ...
    for ( quint16 i = 0; i < u16CLCount; i++ ){
        ...
        riTemp[i].u16NameLength = QString(baTemp.toHex()).toUInt(&ok, 16);
        baTemp.clear();
        baData.remove(0, 2);
        ...
        baTemp.append(baData[0]);
        baTemp.append(baData[1]);
        baData.remove(0, 2);
        riTemp[i].u16Group = QString(baTemp.toHex()).toUInt(&ok, 16);
        qDebug() << "riTemp[" << i << "].u16Group =" << riTemp[i].u16Group;
        baTemp.clear();
        baTemp.append(baData[0]);
        baTemp.append(baData[1]);
        baData.remove(0, 2);
        riTemp[i].u16Id = QString(baTemp.toHex()).toUInt(&ok, 16);
        qDebug() << "riTemp[" << i << "].u16Id =" << riTemp[i].u16Id;
        baTemp.clear();
        baTemp.append(baData[0]);
        baTemp.append(baData[1]);
        baData.remove(0, 2);
        riTemp[i].u16Type = QString(baTemp.toHex()).toUInt(&ok, 16);
        qDebug() << "riTemp[" << i << "].u16Type =" << riTemp[i].u16Type;
        baTemp.clear();
        baTemp.append(baData[0]);
        baTemp.append(baData[1]);
        baData.remove(0, 2);
        riTemp[i].u16DataLength = QString(baTemp.toHex()).toUInt(&ok, 16);
...
{
    QByteArray baData;
    baData.append(u16ToBA(sSN.length()));
    baData.append(sSN);
