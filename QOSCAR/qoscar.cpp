...
QOSCAR::QOSCAR(QObject *parent): QObject(parent)
{
    ...
    connect(&metaInfo, SIGNAL(onMetaInfo(QSNInfo,quint8)),
    ...
    connect(&oservice, SIGNAL(onLoggedIn()), this, SLOT(onLoggedIn()));
...
{
    u64Received += socket.sock.bytesAvailable();
    qDebug() << "Reading data...";
...
void QOSCAR::onMetaInfo(const QSNInfo &info, quint8 type)
{
    emit fMetaInfo(info, type);
    qDebug() << "Proto: Meta Info!";
}
...
void QOSCAR::handleSnac(const QByteArray &fdataArr)
{
    ...
    switch ( snac.familyId() ){
        case FOODGROUP_OSERVICE:
            oservice.handlePacket(snac.data(), snac.familySubType());
            break;

        case FOODGROUP_ICBM:
            ICBM.handlePacket(snac.data(), snac.familySubType());
            break;

        case FOODGROUP_FEEDBAG:
            qrRoster.handlePacket(snac.data(), snac.familyId(), snac.familySubType());
            break;

        case FOODGROUP_MDIR:
            metaInfo.handlePacket(snac);
    }
...
{
    sendFlap(SNAC_CHANNEL, metaInfo.createIMD__REQUEST_AUTH_INFO(screenName(), bWA, bAuth),
             false);
}
...
void QOSCAR::sendFlap(char cChannel, const QByteArray &baData, bool bHello)
{
    ...
    u64Sent += flap.toByteArray(bHello).length();
...
{
    sendFlap(SNAC_CHANNEL, metaInfo.createSEND_BASIC_INFO(snInfo, sScreenName), false);
}
...
/* Requests SN's info */
void QOSCAR::requestInfo(const QString &sSN)
{
    sendFlap(SNAC_CHANNEL, metaInfo.createCLI_METAREQINFO(sSN, sScreenName), false);
}

/* Searching user by SN */
void QOSCAR::searchBySN(const QString &sSN)
{
    sendFlap(SNAC_CHANNEL, metaInfo.createSearchBySN(sSN, screenName()), false);
}
