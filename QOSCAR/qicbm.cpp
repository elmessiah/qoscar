...
QByteArray QICBM::createICBM__MESSAGE_TO_HOST(const QString &SN, const QString &Message)
{
    QByteArray bArr;
...
void QICBM::handleMessage(const QByteArray &baMessage, const QString &sEncoding)
{
    ...
    QTextCodec* codec = QTextCodec::codecForName(QByteArray(sEncoding.toAscii()));

    mResult.sMessage.append(codec->toUnicode(baTemp));
